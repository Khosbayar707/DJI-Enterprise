...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === 'spec') {
      ...
      return CustomResponse(true, 'REQUEST_SUCCESS', 'Эд ангийн категори амжилттай нэмэгдлээ!', {
        new: newCategory,
      });
    }
    if (type === 'model') {
      const newCategory = await prisma.droneModel.create({ data: { name } });
      return CustomResponse(true, 'REQUEST_SUCCESS', 'Модел амжилттай нэмэгдлээ!', {
        new: newCategory,
      });
    }
    const newCategory = await prisma.droneCategory.create({ data: { name } });
    return CustomResponse(true, 'REQUEST_SUCCESS', 'Дроны категори амжилттай нэмэгдлээ!', {
      new: newCategory,
    });
  } catch (err) {
