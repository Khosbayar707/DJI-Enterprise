...
export default function DeleteSurveyEquipment({ productId, setRefresh }: Props) {
  const [loading, setLoading] = useState(false);

  const handleDelete = async () => {
    if (!confirm('Та энэ хэмжилтийн багажийг устгахдаа итгэлтэй байна уу?')) return;
    setLoading(true);
    try {
      await axios.delete(`/api/hitargets/hitarget?id=${productId}`);
      setRefresh((prev) => !prev);
    } catch (error) {
      console.error('Error deleting survey equipment:', error);
      alert('Хэмжилтийн багажийг устгахад алдаа гарлаа.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <button
      ...
      title="Хэмжилтийн багажийг устгах"
      aria-label="Хэмжилтийн багажийг устгах"
    >
      {loading ? (
