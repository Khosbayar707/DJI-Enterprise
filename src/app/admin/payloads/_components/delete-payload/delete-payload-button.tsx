...
export default function DeletePayload({ payloadId, setRefresh }: Props) {
  const [loading, setLoading] = useState(false);

  const handleDelete = async () => {
    if (!confirm('Та энэ Payload-ийг устгахдаа итгэлтэй байна уу?')) return;
    setLoading(true);
    try {
      await axios.delete(`/api/payloads/payload?id=${payloadId}`);
      setRefresh((prev) => !prev);
    } catch (error) {
      console.error('Payload устгахад алдаа гарлаа:', error);
      alert('Payload устгахад алдаа гарлаа.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <button
      ...
      title="Payload устгах"
      aria-label="Payload устгах"
    >
      {loading ? (
