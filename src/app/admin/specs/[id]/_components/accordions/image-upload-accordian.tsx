...
const ImageUploadAccordion = ({ setRefresh, id }: Props) => {
  ...
  const imageUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response1 = await axios.get(`/api/auth/cloudinary-sign?folder=Spec/Images`);
      ...
      for (const file of files) {
        ...
        data.append('folder', 'Spec/Images');
  ...
  const handleSubmit = async () => {
    try {
      setImageUploading(true);
      const res = await axios.post('/api/product/specs/image', {
        url: ImagePreview,
        public_id: publicIds,
        id,
      });
